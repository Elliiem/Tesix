...
void TESIX_FileWindow::MoveCur(uint32_t line, uint32_t col) {
    SetCur(cur.line + line, cur.col + col);
}
...
void TESIX_FileWindow::Action(TESIX_KeyPress key) {
    if (key.key == "KEY_DOWN") {
        if (cur.line < file->Len() - 1) {
            if (cur.col > file->LineTextLen(cur.line + 1)) SetCur(TESIX_Location(cur.line + 1, file->LineTextLen(cur.line + 1)));
            else MoveCur(1, 0);
            UpdateView();
        }
        ...
        if (cur.col <= 0 && cur.line != 0) SetCur(TESIX_Location(cur.line - 1, file->LineTextLen(cur.line - 1)));
        else MoveCur(0, -1);
        UpdateView();
    } else if (key.key == "KEY_RIGHT") {
        if (cur.col >= file->LineTextLen(cur.line) && cur.line < file->Len() - 1) SetCur(TESIX_Location(cur.line + 1, 0));
        else if (cur.col < file->LineTextLen(cur.line)) SetCur(TESIX_Location(cur.line, cur.col + 1));
        UpdateView();
    } else if (key.key == "KEY_BACKSPACE") {
        if (cur.col == 0) {
            if (file->LineTextLen(cur.line) != 0 && cur.line != 0) {
                TESIX_Location start(cur.line, 0);
                TESIX_Location end(cur.line, file->LineTextLen(cur.line) - 1);
                TESIX_Location target(cur.line - 1, file->LineTextLen(cur.line - 1));
                ...
                UpdateView();
            } else if (file->LineTextLen(cur.line) == 0) {
                TESIX_Editor::DelLine(cur.line, file);
                SetCur(cur.line - 1, file->LineTextLen(cur.line - 1));
                UpdateView();
            }
        } else if (cur.col <= file->LineTextLen(cur.line)) {
