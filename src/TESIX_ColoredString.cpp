...
bool TESIX_Color::operator==(TESIX_Color& other) { return other.GetBase() == base && other.GetCurrent() == cur; }
...
uint32_t TESIX_Color::GetCurrent() { return cur; }

uint32_t TESIX_Color::GetBase() { return base; }
...
TESIX_ColorIndexPair::TESIX_ColorIndexPair() { index = 0; }
...
TESIX_ColoredString::TESIX_ColoredString(std::string str, TESIX_Color color) {
    next_cur_index = 0;
    this->colored_string = str;
    colors.push_back(TESIX_ColorIndexPair{color, 0});
}
...
uint32_t TESIX_ColoredString::LastColoredIndex() { return colors[colors.size() - 1].index; }
...
std::string TESIX_ColoredString::GetStr() { return colored_string; }
