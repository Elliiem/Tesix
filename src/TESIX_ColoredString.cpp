...
std::pair<std::string, uint32_t> TESIX_ColoredString::NextColorSegment(){
    if(!(next_cur_index >= colors.size())){
        ...
        return std::pair<std::string, uint32_t>{ret_str, ColorAt(start)};
    } else {
...
uint32_t TESIX_ColoredString::ColorAt(uint32_t index){
    TESIX_Color color = colors[GetColorIndex(index)].color;
    return color.GetCurrent() * (color.GetCurrent() != TESIX_COLORS_NULL);
}
